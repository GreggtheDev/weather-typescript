...
document.getElementById('weather-form')?.addEventListener('submit', function(e: Event) {
    e.preventDefault();  // Prevent the default form submission behavior to handle it via JavaScript
    const zip = (document.getElementById('zip') as HTMLInputElement).value;  // Get the value of the input field with the ID 'zip'
    getWeatherData(zip);  // Call the function to fetch and display weather data for the provided zip code
});

// Interface to describe the structure of weather data received from the API
...
function getWeatherData(zip: string): void {
    ...
    fetch(url)
        .then(response => {
            if (!response.ok) {  // Check if the response is not okay, if so, throw an error
        ...
        .catch(error => {
            ...
            alert('Failed to fetch weather data. Please check the zip code and try again.');  // Alert the user about the error
        });
}

// Function to display the fetched weather data on the webpage
function displayWeatherData(data: WeatherData): void {
    if (!data || !data.main) {  // Check if the data is invalid or doesn't contain the necessary information
